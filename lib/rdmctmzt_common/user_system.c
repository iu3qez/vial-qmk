...
void es_chibios_user_idle_loop_hook(void) {
    ...
    if (Keyboard_Info.Key_Mode == QMK_USB_MODE) {
        ...
        wait_ms(2);
#else
        register_code(keymap_key_to_keycode(0, (keypos_t){.col = Col_Count, .row = Rol_Count}));
        wait_ms(2);
        unregister_code(keymap_key_to_keycode(0, (keypos_t){.col = Col_Count, .row = Rol_Count}));
        wait_ms(2);
#endif
    }
